...
final class GameQuizView: UIView {
	...
	func setupView() {
		...
		for sunji in gameQuizCard.sunjis.shuffled() {
			let sunjiButton = UIButton().then {
				...
				$0.titleLabel?.font = .Pretendard.m16
	...
	private func didTapSunjiButton(_ sender: UIButton) {
		...
		if isCorrect {
			...
			correctLottieAnimationView.isHidden = false
		} else {
			wrongLottieAnimationView.play()
			wrongLottieAnimationView.isHidden = false
		}
		...
		DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
			...
			self.correctLottieAnimationView.isHidden = true
			self.wrongLottieAnimationView.isHidden = true
